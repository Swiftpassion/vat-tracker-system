...
# --- Apply TH Sarabun Font & Global CSS ---
st.markdown("""
<style>
...
st.title("📊 ระบบจัดการ VAT คงเหลือ")

# เพิ่มเมนูใหม่ตามที่คุณต้องการ
...
# ==========================================
# Menu 3, 4, 5: Reports (Dynamic Shared Component)
# ==========================================
elif menu in ["📈 3. รายงานรวม (ทั้งหมด)", "🏢 4. รายงานบริษัท KIT", "🏢 5. รายงานบริษัท S16"]:
    
    # 1. เช็คว่าเป็นเมนูไหน เพื่อตั้งค่าหน้าจอให้ถูกต้อง
    if "KIT" in menu:
        st.header("🏢 รายงานบริษัท KIT (เฉพาะ KIT Vat)")
        fixed_vat = "KIT"
    elif "S16" in menu:
        st.header("🏢 รายงานบริษัท S16 (เฉพาะ S16 Vat)")
        fixed_vat = "S16"
    else:
        st.header("📈 รายงานรวม (ทั้งหมด)")
        fixed_vat = "ALL"
    
    # --- CSS ควบคุมความสวยงามของตารางและซ่อนขอบฟอร์ม ---
    st.markdown("""
    <style>
    .btn-custom-download {
        ...
        transition: border-color 0.2s, color 0.2s; margin-top: 28px;
    }
    ...
    /* ซ่อนขอบ Form เพื่อให้ UI รวมเป็นเนื้อเดียวกัน */
    ...
    # -------------------------------------------------------------
    # โซนด้านบนสุด: จัดกลุ่มให้เรียงชิดกัน (สถานะ | รุ่นสินค้า | บันทึก | ดาวน์โหลด)
    # -------------------------------------------------------------
    # แถวตัวกรอง (Filters) จะอยู่นอก Form เพื่อให้พิมพ์แล้วค้นหาได้ทันที
    f_col1, f_col2, f_col3 = st.columns([1.5, 2, 4])
    with f_col1:
        stat_map = {"ทั้งหมด": "ALL", "ยังไม่ขาย": "AVAILABLE", "ขายแล้ว": "USED"}
        stat_filter = st.selectbox("สถานะสินค้า", list(stat_map.keys()))
    with f_col2:
        model_search = st.text_input("รุ่นสินค้า (พิมพ์บางส่วน)")

    # สร้าง Query หาข้อมูล
    query = supabase.table('vat_inventory').select('*')
    if stat_map[stat_filter] != "ALL": query = query.eq('status', stat_map[stat_filter])
    if fixed_vat != "ALL": query = query.eq('vat_company', fixed_vat)
    if model_search: query = query.ilike('model', f'%{model_search}%')
    ...
    if not records:
        ...
        # เตรียม Dataframe สำหรับโหลด Excel
        ...
        # ระบบ Pagination (แบ่งหน้า) - เราเอามาเก็บตัวแปรก่อน แต่แสดงผลไว้ข้างล่างสุด
        items_per_page = 100
        ...
        # วาดตารางและปุ่มจัดการ (อยู่ใน Form เพื่อป้องกันเว็บริเฟรช)
        with main_table_area:
            with st.form("report_edit_form"):
                
                # นำปุ่มมาเรียงหลอกตาให้เหมือนอยู่ต่อจากกล่องค้นหา
                action_col1, action_col2, _ = st.columns([1.5, 1.5, 4.5])
                with action_col1:
                    # ดันปุ่มลงมาให้อยู่ระนาบเดียวกับช่องกรอกข้อมูล
                    st.markdown("<div style='height: 28px;'></div>", unsafe_allow_html=True)
                    submitted = st.form_submit_button("💾 บันทึกการแก้ไข", type="primary", use_container_width=True)
                with action_col2:
                    st.markdown(get_excel_download_html(df_dl, f"vat_report_{fixed_vat}.xlsx"), unsafe_allow_html=True)
...
# ==========================================
# Menu 6: Search and Edit (Custom Table UI)
# ==========================================
elif menu == "✏️ 6. ค้นหาและแก้ไขข้อมูล":
    ...
    if status_map[status_filter] != "ALL": query = query.eq('status', status_map[status_filter])
    if vat_map[vat_filter] != "ALL": query = query.eq('vat_company', vat_map[vat_filter])
    if model_search: query = query.ilike('model', f'%{model_search}%')
