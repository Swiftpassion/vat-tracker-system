...
from models import Base, VatCompany, VatInventory, VatStatus

# --- 1. System Setup & Configuration ---
st.set_page_config(page_title="VAT Tracker (KIT & S16)", page_icon="📊", layout="wide")
logger = logging.getLogger(__name__)

# Initialize Database Connection via Streamlit Secrets
@st.cache_resource
def init_db() -> sessionmaker:
    """Initializes the database connection using Supabase URL."""
    db_url = st.secrets.get("DATABASE_URL", "sqlite:///./local_test.db")
...
def convert_df_to_excel(df: pd.DataFrame) -> bytes:
    """Converts a pandas DataFrame to an Excel byte stream for downloading."""
    output = io.BytesIO()
...
if menu == "📥 1. อัปโหลดไฟล์ซื้อเข้า (เพิ่ม VAT)":
    ...
    if purchase_file is not None:
        ...
        if df_cleaned is not None:
            ...
            edit_df['วิธีชำระเงิน'] = "เงินสด" # Default
            edit_df['ธนาคารหรือบริษัท'] = "-" # Default
        ...
        if df_sales is not None:
            ...
            if st.button("✂️ ยืนยันการตัดสต๊อก VAT", type="primary"):
                ...
                for _, row in df_sales.iterrows():
                    ...
                    try:
                        used_date = datetime.strptime(str(row['วันที่']), "%d/%m/%Y").date()
                    except ValueError:
                        used_date = datetime.now().date()

                    # Find AVAILABLE VAT matching this IMEI
                    vat_item = db.query(VatInventory).filter(
                        VatInventory.imei == imei,
                        VatInventory.status == VatStatus.AVAILABLE
                    ).first()
                    
                    if vat_item:
                        vat_item.status = VatStatus.USED
                        vat_item.used_date = used_date
                        vat_item.customer_name = customer
                        updated_count += 1
...
# --- Menu 3: Dashboard & Reports ---
    ...
    # Query all data
    all_data = db.query(VatInventory).all()
    ...
    if not all_data:
        ...
        for item in all_data:
            data_list.append({
                ...
                "รุ่น": item.model,
                "IMEI": item.imei,
                "บริษัท VAT": item.vat_company.value,
                "สถานะ": item.status.value,
                "ราคาทุน": item.cost,
                "วันที่ขาย": item.used_date,
