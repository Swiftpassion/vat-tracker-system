...
# ==========================================
    ...
    # 2. Build Database Query (นำ .limit(100) ออก เพื่อดึงข้อมูลทั้งหมดมาทำ Pagination)
    ...
    if not records:
        ...
        # เตรียม Dataframe สำหรับดาวน์โหลด (รวมข้อมูลทุกหน้า)
        ...
        info_col, page_col, _ = st.columns([3, 1.5, 3])
        with info_col:
            st.success(f"พบข้อมูลทั้งหมด {total_items} รายการ")
        with page_col:
            current_page = st.number_input(f"หน้า (จากทั้งหมด {total_pages} หน้า)", min_value=1, max_value=total_pages, value=1, step=1)

        # ตัดข้อมูลเพื่อแสดงผลเฉพาะหน้าที่เลือก
        start_idx = (current_page - 1) * items_per_page
        end_idx = start_idx + items_per_page
        page_records = records[start_idx:end_idx]

        with st.form("report_edit_form"):
            # CSS บังคับฟอนต์ TH Sarabun และตกแต่งปุ่มดาวน์โหลด HTML แบบ Custom
            st.markdown("""
            <style>
            @import url('https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;500;600;700&display=swap');
            ...
            /* CSS สำหรับปุ่มจำลองให้อยู่บรรทัดเดียวกับปุ่ม Save */
            .btn-custom-download {
                display: flex;
                align-items: center;
                ...
                font-weight: 400;
                font-size: 16px;
                height: 42px; /* ความสูงเท่าปุ่ม Streamlit พอดี */
                width: 100%;
            ...
            # ฟังก์ชันสร้างปุ่มดาวน์โหลด HTML
            import base64
            ...
            # --- จัดเรียงปุ่ม Save และ Download ให้อยู่บรรทัดเดียวกันใน Form ---
            ...
            with action_col2:
                # วางปุ่ม HTML ที่สร้างขึ้นมาติดกับปุ่ม Save
                st.markdown(get_excel_download_html(df_dl, f"vat_report_page{current_page}.xlsx"), unsafe_allow_html=True)
            ...
            # --- หัวตาราง ---
            st.markdown('<div style="background-color:#1f2937; padding:8px; border-radius:6px 6px 0 0; margin-top: 10px;">', unsafe_allow_html=True)
            ...
            # --- ข้อมูลตาราง (วนลูปเฉพาะข้อมูลในหน้าที่เลือก page_records) ---
