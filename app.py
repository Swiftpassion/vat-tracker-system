...
# --- Menu 4: Search and Edit (New UI) ---
    ...
    with col_date:
        # แนะนำให้เติมวงเล็บ (ค.ศ.) เพื่อให้ผู้ใช้ทราบ
        target_date = st.date_input("เลือกวันที่ (ค.ศ.)")
    with col_status:
        status_map = {"ทั้งหมด": "ALL", "ยังไม่ขาย": "AVAILABLE", "ขายแล้ว": "USED"}
        status_filter = st.selectbox("เลือกสถานะ", list(status_map.keys()))

    # ---------------------------------------------------------
    # 🌟 เพิ่ม Logic แปลง ค.ศ. เป็น พ.ศ. สำหรับไปค้นหาในฐานข้อมูล
    # ---------------------------------------------------------
    try:
    ...
    if not records:
        st.warning(f"ไม่พบข้อมูล {search_type} ในวันที่ {target_date.strftime('%d/%m/%Y')} (ค้นหาด้วย พ.ศ. {search_date_db.year})")
