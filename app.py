...
# ==========================================
    ...
    # CSS ตกแต่งให้ columns ดูเหมือนตาราง HTML
    st.markdown("""
    <style>
    .table-header {
    ...
    /* จัดข้อความในคอลัมน์ให้อยู่กึ่งกลางแนวตั้ง */
    ...
    with col_model:
        model_search = st.text_input("รุ่นสินค้า (พิมพ์บางส่วนเพื่อค้นหา)", value="")
        ...
        with st.form("custom_table_form"):
            ...
            if search_type == "การซื้อเข้า":
                ...
                for rec in records:
                    ...
                    # Dropdown วิธีการชำระเงิน
                    pm_opts = [" ", "เงินสด", "โอนเงิน", "ผ่อนธนาคาร", "เครดิตบริษัท"]
                    curr_pm = rec.get('inbound_payment_method', " ")
                    new_pm = cols[6].selectbox(
                        "pm", pm_opts, 
                        index=pm_opts.index(curr_pm) if curr_pm in pm_opts else 0, 
                        key=f"in_pm_{rec['id']}", label_visibility="collapsed"
                    )

                    # Text Input ธนาคารหรือบริษัท
                    curr_bank = rec.get('inbound_bank_or_company', "")
                    new_bank = cols[7].text_input(
                        "bank", value=curr_bank if curr_bank else "", 
                        key=f"in_bnk_{rec['id']}", label_visibility="collapsed"
                    )
                ...
                for rec in records:
                    ...
                    new_pm = cols[6].selectbox(
                        "pm", pm_opts, 
                        index=pm_opts.index(curr_pm) if curr_pm in pm_opts else 0, 
                        key=f"out_pm_{rec['id']}", label_visibility="collapsed"
                    )

                    # Dropdown บริษัทที่รับเงิน
                    comp_opts = [" ", "บริษัท KIT", "บริษัท S16"]
                    curr_comp = rec.get('outbound_receiving_company', " ")
                    new_comp = cols[7].selectbox(
                        "comp", comp_opts, 
                        index=comp_opts.index(curr_comp) if curr_comp in comp_opts else 0, 
                        key=f"out_comp_{rec['id']}", label_visibility="collapsed"
                    )

                    payload.append({"id": rec['id'], "type": "out", "pm": new_pm, "comp": new_comp})
                    st.markdown("<hr style='margin: 5px 0; border-top: 1px solid #f3f4f6;'/>", unsafe_allow_html=True)

            # ปุ่ม Save นอกลูป แต่อยู่ใน Form
