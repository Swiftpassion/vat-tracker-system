...
# ==========================================
    ...
    if not records:
        ...
        # เตรียม Dataframe สำหรับดาวน์โหลด
        ...
        # 4.1 จัดเรียงปุ่มดาวน์โหลดให้อยู่แนวเดียวกับปุ่มบันทึก (โดยให้อยู่บน Form)
        info_col, dl_col = st.columns([4, 1])
        with info_col:
            st.success(f"พบข้อมูล {len(records)} รายการ (แสดงผลสูงสุด 100 รายการ)")
        with dl_col:
        ...
        with st.form("report_edit_form"):
            # ปุ่มบันทึกการแก้ไข อยู่ด้านบนสุดของตาราง
            save_col, _ = st.columns([1, 4])
            with save_col:
                submitted = st.form_submit_button("💾 บันทึกการแก้ไข", type="primary", use_container_width=True)

            # CSS บังคับฟอนต์ TH Sarabun เด็ดขาด และปรับขนาดตาราง
            st.markdown("""
            <style>
            @import url('https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;500;600;700&display=swap');
            ...
            def fmt(val: Any) -> str:
                """Helper function to return space ' ' if value is None, NaN or empty."""
            ...
            with st.container(height=600, border=True):
                for rec in records:
                    ...
                    # ข้อมูล ขาออก (เช็คว่าขายหรือยัง)
