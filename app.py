...
# ==========================================
    ...
    # 2. Build Database Query (ดึงข้อมูลทั้งหมดเพื่อมาทำแบ่งหน้า)
    ...
    if not records:
        ...
        # --- CSS บังคับฟอนต์และดึง Form ขึ้นมาซ้อนทับ (Overlap Hack) ---
        st.markdown("""
        <style>
        @import url('https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;500;600;700&display=swap');
        ...
        .info-badge {
            background-color: #1f2937; color: #39ff14; padding: 8px 12px;
            border-radius: 6px; font-weight: bold; font-size: 15px;
            display: flex; align-items: center; justify-content: center;
            height: 40px; margin-top: 28px; /* จัดให้อยู่ตรงกลางระดับเดียวกับช่องกรอกหน้า */
        }
        .btn-custom-download {
            display: flex; align-items: center; justify-content: center;
        ...
        /* 1. ซ่อนขอบและดึง Form ขึ้นไปซ้อนทับแถวด้านบนเป๊ะๆ (-105px คือความสูงเฉลี่ยของ 1 แถว) */
        ...
        /* 2. ทำให้ช่องว่างของ Form ทะลุการคลิกได้ เพื่อไม่ให้ไปบล็อกปุ่มเปลี่ยนหน้าด้านหลัง */
        div[data-testid="stForm"] > div > div > div > div:nth-child(1),
        div[data-testid="stForm"] > div > div > div > div:nth-child(2) {
        ...
        # -------------------------------------------------------------
        # แถวควบคุมด้านบน (โซนซ้าย: อยู่นอก Form เพื่อให้เปลี่ยนหน้าได้ทันที)
        # -------------------------------------------------------------
        top_col1, top_col2, top_col3, top_col4 = st.columns([1.5, 1.5, 1.5, 1.5])
        with top_col1:
            st.markdown(f"<div class='info-badge'>พบข้อมูลทั้งหมด {total_items} รายการ</div>", unsafe_allow_html=True)
        with top_col2:
        ...
        # -------------------------------------------------------------
        # แถวควบคุมด้านล่าง (โซนขวา: อยู่ใน Form แต่ถูก CSS ดึงขึ้นไปซ้อนทับด้านบน)
        # -------------------------------------------------------------
        with st.form("report_edit_form"):
            f_col1, f_col2, f_col3, f_col4 = st.columns([1.5, 1.5, 1.5, 1.5])
            # ช่อง 1 และ 2 ปล่อยว่างไว้เพื่อเว้นที่ให้ info-badge และ pagination โผล่ขึ้นมา
            
            with f_col3:
                # ดันปุ่มลงมานิดนึงเพื่อให้ตรงกับกล่องกรอกหน้าเว็บพอดี
            ...
            # --- เว้นระยะห่างให้สวยงามก่อนขึ้นหัวตาราง ---
