...
# ==========================================
# Menu 3, 4, 5: Dashboard & Reports (Custom UI - แบบที่คุณเลือก)
# ==========================================
elif menu in ["📈 3. รายงานรวม (ทั้งหมด)", "🏢 4. รายงานบริษัท KIT", "🏢 5. รายงานบริษัท S16"]:
    
    # กำหนดหัวข้อและบริษัทที่ต้องการกรอง
    if "KIT" in menu:
        st.header("🏢 รายงานบริษัท KIT")
        fixed_vat = "KIT"
    elif "S16" in menu:
        st.header("🏢 รายงานบริษัท S16")
        fixed_vat = "S16"
    else:
        st.header("📈 รายงานสถานะ VAT")
        fixed_vat = "ALL"
    
    # 1. Filters (แสดงฟิลเตอร์ตามเมนูที่เลือก)
    if fixed_vat == "ALL":
        ...
        f_col1, f_col3 = st.columns([1.5, 3.5])
        with f_col1:
            stat_map = {"ทั้งหมด": "ALL", "คงเหลือ": "AVAILABLE", "ขายแล้ว": "USED"}
            stat_filter = st.selectbox("สถานะสินค้า", list(stat_map.keys()))
        with f_col3:
            model_search = st.text_input("รุ่นสินค้า (พิมพ์บางส่วน)")
        vat_map = {"ทั้งหมด": "ALL", "บริษัท KIT Vat": "KIT", "บริษัท S16 Vat": "S16"}
        vat_filter = "ทั้งหมด" # จะถูก Override ด้วย fixed_vat ใน Query
    ...
    if fixed_vat != "ALL":
        query = query.eq('vat_company', fixed_vat)
    elif vat_map[vat_filter] != "ALL":
    ...
    if not records:
        ...
        # --- CSS บังคับฟอนต์และตกแต่งปุ่มดาวน์โหลด (ของเดิมตามที่คุณให้มา) ---
        st.markdown("""
        <style>
        @import url('https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;500;600;700&display=swap');
